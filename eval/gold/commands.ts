...
function report(profile: CorpusProfile): void {
  const { totals, confusions, defects } = profile;
  ...
  console.log("Corpus defects — signals a gold standard must not treat as truth");
  console.log(`  non-owner slugs in the user/ namespace: ${defects.ownerNamespaceMisuse.join(", ") || "none"}`);
  console.log(`  declared email threads whose messages share no entity: ${defects.incoherentEmailThreads.length}`);
  console.log(`  meetings whose linked_calendar event disagrees: ${defects.linkedCalendarMismatch.length}`);
  for (const entry of defects.linkedCalendarMismatch) {
    console.log(`    ${entry.meeting} [${entry.meetingAttendees.join(", ")}] -> ${
      entry.event} [${entry.eventAttendees.join(", ")}]`);
  }
  console.log(`  note topics regenerated on several days: ${defects.recurringNoteTopics.length}`);
}
