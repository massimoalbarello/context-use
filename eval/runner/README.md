...
- `longmemeval/` composes isolated corpus distillation with fresh-session QA and ports the
  benchmark's published model-judge rubric.
...
To add an eval, create `eval/data/<id>/` with its fixed inputs or a pinned external-data
manifest and answer key. A
