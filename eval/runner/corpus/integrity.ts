 ...
 * byte under `data/<id>/corpus/` is covered by `data/<id>/corpus.lock.json`. Where
 * upstream ships its own per-item hashes the loader checks those too, and the two together
 * leave nothing unverified: for `amara-life-v1` the manifest proves the vendored notes and
 * meetings are upstream's, and the lockfile proves nothing has changed since vendoring.
 */
...
export const DATA_ROOT = join(import.meta.dir, "..", "..", "data");
...
export function corpusDirectory(id: CorpusId): string {
  return join(DATA_ROOT, id, "corpus");
}

export function corpusLockPath(id: CorpusId): string {
  return join(DATA_ROOT, id, "corpus.lock.json");
}

export function qaDirectory(id: CorpusId): string {
  return join(DATA_ROOT, id, "qa");
}
...
export function corpusIdAt(directory: string): CorpusId {
  const resolved = directory.replace(/\/+$/, "");
  const leaf = basename(resolved);
  const name = leaf === "corpus" ? basename(dirname(resolved)) : leaf;
  if (!isCorpusId(name)) {
