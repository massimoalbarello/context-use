...
try {
  ...
  if (!args[0] || args[0] === '--help') {
    console.log(
      'context-use-openclaw connect <instance-url> [agent-id=main]\ncontext-use-openclaw authorize <redirect-url-file|->\ncontext-use-openclaw status\ncontext-use-openclaw remove\n\nConnect installs through OpenClaw. Authorize reads the callback from a private file or stdin. After authorization, all commands are also available through openclaw context-use.',
    );
  } else {
