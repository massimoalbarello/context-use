...
- Context-use, not this automation, records the page-change ledger. Never reconstruct
  it from diary pages, page prose, timestamps or agent recollection. Never write a
...
Call `prepare_knowledge_write` for each row's path to load its current root-to-leaf guide
chain. Use the returned guidance for review only; never pass its receipt to a mutation
because this automation is read-only.
