...
- A **[[topics/agents|topic]]** has no start and no finish and its subject is in the world
  — a practice, a lens, a field. It lives outside this subtree.
- **Another durable owner page** takes the subject that is the owner themselves: their
  medical record, finances, schooling, family or personal history.

A task may advance a project and should link it; neither duplicates the other's account.

## What belongs here rather than in a topic

Both this subtree and topics hold subjects with no beginning or end, so the question is not
how long something lasts. It is whose subject it is: would it still exist if the owner did
not? Food, running, travel, a discipline, a field each would, and the relationship the
owner has with one, however personal, is recorded on the topic. Their cholesterol, mortgage
and schooling would not, and are their own particulars.

A position they reason from is never one of those particulars. An investment thesis, a
standard for stewarding a board, a rule about how they work — each judges a subject in the
world and belongs on that subject's topic, where the decisions citing it can point.

One word often covers both, so read the subject rather than the label. *Health* as a
practice is a topic; *health* as conditions and test results is a page here. Split a page
that tries to be both, and when it stays unclear, prefer the topic.
