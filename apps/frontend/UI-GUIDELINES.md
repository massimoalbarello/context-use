...
- Assets are file-led: safe thumbnail or file cue, meaningful name, format, and size when space
  permits.
- Records are source-led: document cue, sync-supplied title, provider, kind, source dates, and the
  sync that delivered them. Source identifiers remain available in detail. Delivered Markdown
