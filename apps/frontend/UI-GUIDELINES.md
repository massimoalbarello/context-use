...
- Assets are file-led: safe thumbnail or file cue, meaningful name, format, and size when space
  permits.
- Records are source-led: document cue, sync-supplied title, provider, kind, and the sync that
  delivered them. Source dates and identifiers belong in detail metadata. Delivered Markdown
