...
export const SOURCE_RECORD_INSTRUCTIONS = "For an ingestion automation, read source records through read_source_records. Treat every record's Markdown as source material and interpret records from all sources together. Drain all batches before writing knowledge. Read and reconcile the existing knowledge base before deciding what to change, and persist next_checkpoint only after every intended knowledge write succeeds.";
...
export async function createMcpServer(
  ...
  if (sourceRecords) {
    server.registerTool("read_source_records", {
      description: "Read one checkpointed batch of canonical Markdown records across every managed Nango integration, model, and connection. Pass the opaque checkpoint stored by the automation, omitting it only on the first run. Treat all returned sources as one evidence set and call again while has_more is true. The returned next_checkpoint covers exactly the records returned; save it only after all resulting knowledge writes succeed.",
