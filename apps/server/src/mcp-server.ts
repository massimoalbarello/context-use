...
export const SOURCE_RECORD_INSTRUCTIONS = "For an ingestion automation, one scheduled invocation may call read_source_records repeatedly to drain the available backlog while enough model context remains for both another read and the complete knowledge reconciliation. Pass each returned next_checkpoint into the next read while has_more is true. Before every read, choose max_bytes from the context currently available rather than applying a fixed run quota, then use batch_bytes to assess whether another read still fits. A newly discovered source stream begins with records modified during the preceding 30 days; the activity described by a returned record may be older and is processed normally. Treat all records accumulated across sources and reads as one evidence set, and use each action to distinguish current evidence from a deletion. Read and reconcile the existing knowledge base before deciding what to change. Persist only the last next_checkpoint, and only after every intended knowledge write for the accumulated evidence succeeds. If available context stops the drain while has_more is true, resume from that saved checkpoint on the next scheduled invocation.";
    ...
    server.registerTool("read_source_records", {
      ...
      inputSchema: z.object({
        ...
        max_bytes: z.number().int().min(1)
          .describe("Target maximum serialized bytes chosen for this read from the model context currently available. One individually larger record may exceed it so the checkpoint can advance."),
      }).strict(),
