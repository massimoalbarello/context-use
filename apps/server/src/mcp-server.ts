...
export async function createMcpServer(
  ...
  const server = new McpServer({ name: "context-use", version: "0.1.47" }, {
  ...
  server.registerTool("create_directory", {
    description: "Create a first-class directory beneath an existing directory. Before calling, use prepare_knowledge_write with the new directory path and follow every returned AGENTS.md guide. Its title and optional summary are public-listing metadata; the directory becomes immediately linkable by path or stable directory reference.",
  ...
  server.registerTool("update_directory", {
    description: "Edit a directory's public-listing title, optional summary, and private Markdown introduction. Before calling, use prepare_knowledge_write with the directory's current path and follow every returned AGENTS.md guide. Public child listings are generated by the framework.",
