...
create table "record_delivery_batch" (
...
create table "record_event" (
  ...
  foreign key ("sync_id", "owner_id")
    references "record_sync" ("id", "owner_id") on delete cascade,
  foreign key ("sync_id", "batch_id")
    references "record_delivery_batch" ("sync_id", "batch_id") on delete cascade,
  ...
  check ("event_fingerprint" not glob '*[^a-f0-9]*' and length("event_fingerprint") = 64),
  check (length(trim("received_at")) > 0)
);
...
create table "record" (
  ...
  foreign key ("sync_id", "owner_id")
    references "record_sync" ("id", "owner_id") on delete cascade,
  foreign key ("sync_id", "current_event_id")
    references "record_event" ("sync_id", "event_id"),
  check (length(trim("provider")) > 0),
  check (length("readable_id") between 1 and 120),
  check (substr("readable_id", 1, 1) glob '[a-z0-9]'),
  check ("readable_id" not glob '*[^a-z0-9-]*'),
...
create table "record_ingestion_job" (
  "sync_id" text not null,
  "owner_id" text not null,
  "event_id" text not null,
  "source_id" text not null,
  ...
  foreign key ("sync_id", "owner_id")
    references "record_sync" ("id", "owner_id") on delete cascade,
  foreign key ("sync_id", "event_id")
    references "record_event" ("sync_id", "event_id") on delete cascade,
...
create index "record_delivery_batch_owner_received_idx"
  on "record_delivery_batch" ("owner_id", "received_at" desc);
create index "record_owner_kind_idx" on "record" ("owner_id", "provider", "kind", "updated_at" desc);
create index "record_event_revision_idx"
  on "record_event" ("sync_id", "source_id", "kind", "record_id", "revision");
create index "record_ingestion_job_due_idx"
  on "record_ingestion_job" ("state", "available_at", "lease_expires_at", "created_at");
