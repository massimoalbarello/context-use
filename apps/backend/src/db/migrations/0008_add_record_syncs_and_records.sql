...
-- Publication and delivery control only. Canonical records live in immutable storage files.
create table "record_delivery_head" (
  ...
  check (substr("readable_id", 1, 1) glob '[a-z0-9]'),
  check ("readable_id" not glob '*[^a-z0-9-]*'),
  check ("identity_key" not glob '*[^a-f0-9]*' and length("identity_key") = 64),
  check ("revision" between 1 and 9007199254740991),
  check ("operation" in ('added', 'updated', 'deleted')),
  check ("revision_hash" not glob '*[^a-f0-9]*' and length("revision_hash") = 64),
  check ("blob_hash" not glob '*[^a-f0-9]*' and length("blob_hash") = 64),
