...
export type OpenConnectorIngestionTickResult =
  | 'idle'
  | 'completed'
  | 'superseded'
  | 'lost_lease'
...
function storedError(error: unknown): string {
  ...
  return (message || 'Unknown open-connector ingestion failure').slice(0, MAX_STORED_ERROR_LENGTH);
}

function projectionFor(job: OpenConnectorIngestionJob): OpenConnectorJobProjection | null {
...
export class OpenConnectorIngestionWorker {
  ...
  private readonly logger = createLogger('OpenConnectorIngestionWorker');
  ...
  async tick(): Promise<OpenConnectorIngestionTickResult> {
  ...
  private async run(signal: AbortSignal): Promise<void> {
    while (!signal.aborted) {
      try {
        ...
        this.logger.error('Open-connector ingestion tick failed', error);
      }
