...
const RecordContentSchema = z
  ...
  .refine(
    (content) => Buffer.byteLength(JSON.stringify(content), 'utf8') <= MAX_RECORD_CONTENT_BYTES,
    `Record content must be no larger than ${MAX_RECORD_CONTENT_BYTES} bytes.`,
  );
...
export const RecordDeliveryEnvelopeSchema = z
  .strictObject({
    version: z.literal(RECORD_DELIVERY_VERSION),
    ...
    records: z
      ...
      .min(1)
      .max(MAX_RECORD_DELIVERY_BATCH_RECORDS),
  })
  .refine(
    (envelope) =>
