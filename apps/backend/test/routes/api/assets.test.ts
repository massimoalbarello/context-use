...
test('assets are server-inspected, linked or assigned, and archived only when unused', async () => {
  ...
  try {
    ...
    const filteredAssetPagesResponse = await app.handle(
      new Request(
        'http://localhost/api/hypermedia/pages?layer=undated&resources=asset:quarterly-chart&kinds=entity,asset&query=Quarterly%20chart',
      ),
    );
    ...
    const entityOnlyAssetPagesResponse = await app.handle(
      new Request(
        'http://localhost/api/hypermedia/pages?layer=undated&resources=asset:quarterly-chart&kinds=entity&query=Quarterly%20chart',
      ),
    );
