...
const SYNC_ID = '01991f43-0c00-7000-8000-000000000010';
...
async function insertSync({ database, timestamp }: { database: SQL; timestamp: string }) {
  await database`
    insert into "record_sync"
      ("id", "owner_id", "readable_id", "name", "api_key_sha256", "created_at")
    values
      (${SYNC_ID}, ${OWNER_ID}, 'worker-sync', 'Worker sync', ${digest('worker-sync-key')},
       ${timestamp})
  `;
}

test('leased ingestion survives restart and fences stale updates and deletions', async () => {
  const dataFolder = await mkdtemp(join(tmpdir(), 'context-use-record-worker-'));
  ...
  try {
    ...
    await insertSync({ database, timestamp: now().toISOString() });
    let repository = new RecordsRepository(database);
    let service = new RecordsService({ records: repository, now });
    expect(await repository.hasUnfinishedJobs()).toBe(false);
    let worker = new RecordIngestionWorker({
      records: repository,
      now,
      leaseToken,
    ...
    await runMigrations({ db: database });
    repository = new RecordsRepository(database);
    service = new RecordsService({ records: repository, now });
    worker = new RecordIngestionWorker({
      records: repository,
      now,
      leaseToken,
      leaseMilliseconds: LEASE_MILLISECONDS,
    ...
    const recoveryWorker = new RecordIngestionWorker({
      records: repository,
      now: advancingNow,
      leaseToken,
      leaseMilliseconds: LEASE_MILLISECONDS,
