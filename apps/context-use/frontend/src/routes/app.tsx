...
export const Route = createFileRoute('/app')({
  beforeLoad: async ({ context, location }) => {
    ...
    const setupPath = location.pathname === '/app/setup';
    const mcpAuthorizationPath = location.pathname === '/app/mcp/authorize';
    const firstEntityPath = location.pathname === '/app/entities/new';
    if (session && !profile && !setupPath && !mcpAuthorizationPath && !firstEntityPath) {
      throw redirect({ to: '/app/setup', search: { redirect: location.href } });
    }
...
function RouteComponent() {
  ...
  const loginPage = useRouterState({ select: (state) => state.location.pathname === '/app/login' });
      ...
      >
        <div className="mx-auto w-full max-w-xl">
          ...
          {profile ? (
            ...
            <Link className={buttonVariants({ size: 'lg', className: 'mt-7' })} to="/app/login">
              Go to sign in
            </Link>
          )}
        </div>
