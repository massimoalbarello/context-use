...
import { passkeyErrorMessage } from '../src/lib/passkey-error';

const fallback = 'Could not create your passkey.';
...
test.each([
  ...
  const message = passkeyErrorMessage({ error, fallback });
...
test('preserves unrelated errors instead of suggesting a domain change', () => {
  ...
  expect(passkeyErrorMessage({ error: { code: 'AUTH_CANCELLED', message }, fallback })).toBe(
    message,
  );
});

test('supplies a fallback when registration fails without a message', () => {
  expect(passkeyErrorMessage({ error: {}, fallback })).toBe('Could not create your passkey.');
});
