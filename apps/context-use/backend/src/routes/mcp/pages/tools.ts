...
export function registerKnowledgePageTools({
  ...
  server.registerTool(
    'update_knowledge_page',
    {
      ...
      description: `Create a new private revision without changing the published revision. First call read_publication_status; an active publication requires informed user confirmation of this edit, even when the latest revision is private. Existing explicit informed confirmation counts. expectedRevisionNumber is required to prevent overwriting concurrent changes. ${TEMPORAL_COVERAGE_UPDATE_DESCRIPTION}`,
