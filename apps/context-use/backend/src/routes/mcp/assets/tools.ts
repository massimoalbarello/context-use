...
import {
  ...
  mcpAssetSummary,
  mcpAssetUsage,
} from '#backend/routes/mcp/assets/model.ts';
import type { AssetTransferCapabilitiesContract } from '#backend/routes/mcp/assets/transfer-capabilities.ts';
import { MCP_ASSET_TRANSFER_CAPABILITY_HEADER } from '#backend/routes/mcp/assets/transfer-capabilities.ts';
import { AssetAddressSchema } from '#backend/routes/mcp/coordinates.ts';
import {
  DEFAULT_MCP_LIST_LIMIT,
  decodeMcpCursor,
  encodeMcpCursor,
  McpListInputSchema,
} from '#backend/routes/mcp/pagination.ts';
import {
  MCP_ARCHIVE_TOOL_ANNOTATIONS,
  MCP_READ_TOOL_ANNOTATIONS,
  MCP_WRITE_TOOL_ANNOTATIONS,
} from '#backend/routes/mcp/tool-annotations.ts';
...
export function registerAssetTools({
  ...
  server.registerTool(
    'read_asset',
    {
      title: 'Read asset',
      description:
        'Read metadata, usages, and people this image depicts (detected or confirmed) for one active asset by its exact address. Asset bytes are immutable; request a short-lived download only when bytes are needed, and do not infer unsupported identity, location, intent, or chronology from them.',
