...
import type { command as authRecoverpasskeyCmd } from './commands/auth/recover-passkey.ts';
...
export const commandTree: RuntimeNode = {
  ...
  literalChildren: {
    'auth': {
      segment: { kind: 'literal', value: 'auth' },
      command: null,
      literalChildren: {
        'recover-passkey': {
          segment: { kind: 'literal', value: 'recover-passkey' },
          command: { path: 'auth recover-passkey', load: () => import('./commands/auth/recover-passkey.ts').then((m) => m.command) },
          literalChildren: {},
          paramChild: null,
        },
      },
