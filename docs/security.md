...
- `context_use_projection_owner` owns the five projection views and the pure lossy Markdown projection function; it can select only the knowledge and asset fields those projections require. Its function exposes no arbitrary projection arguments: a direct caller can request only the same explicitly published page already visible through the public view.
