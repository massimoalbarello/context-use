...
Migrations bootstrap only a private root `AGENTS.md` structure guide. It tells agents to create `about/intro` if missing, keep it private by default, and ask the owner to review and publish it if they want the landing page to introduce them. It also identifies `skills/<skill-name>` as the semantic folder for complete standard `SKILL.md` page bodies. The guide has no special database authority or lifecycle rules; publishing it requires the same passkey flow as any owner-authored page. Until `about/intro` is published, that route returns a built-in empty-state document rather than private content.
...
- `context_use_mcp`: page reads/writes (including page-backed skills), asset metadata reads, insert-only asset upload intents, narrowly column-scoped automation creation, and automation claiming/completion; no asset update/delete, automation definition updates, or publication.
...
Skills use no separate database or authorization model. Each is an ordinary page at `skills/<skill-name>` whose complete body is a standard `SKILL.md`; the root guide defines discovery, and ordinary page history, commit attribution, dashboard editing, and MCP reads/writes provide its lifecycle. Skills are never attached to automations.

`create_automation` creates a cron-triggered automation with a private instruction page at `automations/<automation-key>/instructions`. The schedule stores that page's stable ID, and each claim reads its current Markdown. A claimed MCP worker may update automation-created pages through its leased page capability, but cannot update schedule fields. Expired or completed claims cannot use run-scoped mutations.

Every automation has an immutable `automations/<automation-key>` knowledge prefix. The semantic key is unique and immutable, while the automation UUID remains provenance metadata. Optional output creation and run-scoped page updates require a random claim token, the matching OAuth client identifier, an active lease, and a relative path. While that client holds an active claim, generic page mutations are denied. Database ownership constraints prevent unrelated pages from occupying the reserved automation tree.

Automation output follows the ordinary page lifecycle after creation. Dashboard and MCP page operations can create new private versions, move it outside the reserved tree, or archive it while unpublished. Only the owner dashboard may publish output, using the version-bound, passkey-confirmed publication flow. Instruction pages may be edited and versioned but are permanently private: both the application and a database trigger reject publication. MCP has no publication capability for any page.
