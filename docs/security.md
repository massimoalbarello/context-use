...
- `context_use_public`: `SELECT` only on `published_pages` and `published_assets` security-barrier views plus execution of the pure lossy Markdown projector used by those views.
- `context_use_public_mcp`: `SELECT` only on the lossy `public_mcp_pages` security-barrier view, execution of that same read-only projector, and column-scoped `INSERT` on confidential inbound messages; no message reads, owner selection, other base-table, webpage-view, asset, or mutable/business-function capability.
...
- `context_use_projection_owner` owns the five projection views and the pure lossy Markdown projection function; it can select only the knowledge and asset fields those projections require.
...
Public views select the exact published page version, not the current private version. Page and asset publication states are independent. Every anonymous page or asset route is under `/p/` and resolves only through those views, so guessing a private path returns the same `404` as a missing path. Before either anonymous process can read Markdown, a database-owned projection removes UUIDs, version metadata, private source paths, comments, scripts/styles, raw HTML, legacy dashboard routes, and private link targets. Independently published link and asset targets become public paths; private targets become inert labels. The public renderer has no private identifier lookup capability.
...
The main private HTTP process holds only `context_use_dashboard`, its storage capability, and the two narrow caller capabilities required to ask auth to validate a dashboard request and confirmation to issue/claim a ceremony. It does not hold the auth, MCP, public, confirmation, storage, backup, PostgreSQL-admin, Better Auth signing, MCP capability-signing, confirmation-gateway, S3, or SSM credential. Authentication authority, private MCP, public rendering, WebAuthn confirmation, and object storage are separate least-privilege processes. The unprivileged auth edge is the only auth process attached to Caddy and shares only a dedicated network with the authority. Each database role has a separate internal network shared only with PostgreSQL; authority calls use distinct edge, dashboard, MCP, and confirmation networks. A common database network cannot become a lateral service bus, and every internal endpoint rejects callers missing its pairwise capability. Caddy sends dashboard-owned auth/settings/confirmation routes to the dashboard, not the authority; confirmation and the authority have no Caddy-facing network, and the dashboard process cannot access a more privileged pool directly.
