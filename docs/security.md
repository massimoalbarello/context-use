...
| Anonymous public MCP client | Sanitized published page Markdown and published hierarchy | Nothing | Read assets or private tables, call application functions, or use any private MCP capability |
...
| Owner-authorized private MCP client | Private context allowed by its OAuth scopes | Author private pages/assets/skills/automations and execute automations within its scopes | Read auth data, publish, export, or obtain any anonymous-server-only capability |
...
| Authentication authority | The single-owner account, passkey, sessions, OAuth clients, grants, and tokens | Maintain authentication and authorization state | Attach to Caddy, hold a knowledge database credential, read knowledge data, or publish/export content |
...
The private MCP is therefore not anonymous. Only the owner can complete its OAuth authorization flow and grant scopes to a client. The public MCP is deliberately anonymous, and its database identity can perform only one business operation: read the lossy published-page projection.
...
- `context_use_public_mcp`: `SELECT` only on the lossy `public_mcp_pages` security-barrier view and execution of a separate public-path-only projector that also removes published-asset tokens; no execution of the webpage projector, base-table, webpage-view, asset, write, or other business-function capability.
...
| Public MCP | `context_use_public_mcp` | None | Anonymous published-page tools |
