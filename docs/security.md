...
| Authentication service | The single-owner account, passkey, sessions, OAuth clients, grants, and tokens | Maintain authentication and authorization state | Hold a knowledge database credential, read knowledge/inbox data, or publish/export content |
...
- `context_use_storage`: column-scoped reads of asset identity, object key, filename, type, size, hash, and deletion state plus `published_assets`; no knowledge, inbox, auth, or metadata writes.
...
- `context_use_projection_owner` owns the three public views and can select only the knowledge and asset base tables those views project.
...
The migration container alone uses the database administrator. No long-running service receives that credential, and there is no combined production application process: dashboard, auth, private MCP, public web, confirmation, storage, public MCP, and backup start independently.

### Long-running process inventory

| Process | Database identity | Other private capability | Publicly routed work |
|---|---|---|---|
| Dashboard | `context_use_dashboard` | Dashboard-only storage, dashboard→auth, and dashboard→confirmation capabilities | Private CRUD, intent creation, export streaming |
| Auth | `context_use_auth` | Better Auth secret, owner bootstrap configuration, two inbound caller capabilities, and confirmation-gateway capability | Passkey/session/OAuth endpoints, dashboard auth-owned settings, and browser confirmation ingress |
| Private MCP | `context_use_mcp` | MCP capability-signing, MCP-only storage, and MCP→auth capabilities | Authenticated MCP and exact asset capability routes |
...
The main private HTTP process now holds only `context_use_dashboard`, its storage capability, and the two narrow caller capabilities required to ask auth to validate a dashboard request and confirmation to issue/claim a ceremony. It does not hold the auth, MCP, public, confirmation, storage, backup, PostgreSQL-admin, Better Auth signing, MCP capability-signing, confirmation-gateway, S3, or SSM credential. Authentication, private MCP, public rendering, WebAuthn confirmation, and object storage are separate least-privilege processes. Each database role has a separate internal network shared only with PostgreSQL; auth calls use three pairwise networks, and dashboard-to-confirmation calls use a fourth. A common database network cannot become a lateral service bus, and every internal endpoint rejects callers missing its pairwise capability. Caddy routes browser confirmation to auth and all other public paths directly to the service that owns them; confirmation has no Caddy-facing network, and the dashboard process cannot dispatch to a more privileged in-process pool.
