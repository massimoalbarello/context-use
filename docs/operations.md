...
Production does not run the repository's convenience combined server. Caddy routes dashboard, auth, private MCP, public web, and public MCP requests to separate containers. Browser confirmation enters through auth, which validates and strips the reusable session/CSRF credentials before calling the internal-only confirmation container. The dashboard container receives only its database URL, dashboard storage token, and narrow dashboard→auth/confirmation caller capabilities; it receives no other pool or signing credential. Object access is provided by a Unix-socket-only broker. Storage and backup are bridge-networked, cannot reach IMDS, and consume separate short-lived bucket-role credentials. The only host-networked container is an input-free credential refresher; it assumes those roles from the instance profile and writes distinct read-only credential volumes.
