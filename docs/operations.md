...
Production does not run the repository's convenience combined server. Caddy routes dashboard, private MCP, public web, and public MCP requests to separate containers; authentication protocol traffic goes only to an unprivileged edge with no database or signing credential. The edge forwards allowlisted protocol routes over a dedicated network to the internal auth authority. Browser confirmation enters through the dashboard and a pairwise dashboard→auth capability; the authority validates and strips reusable session/CSRF credentials before calling the internal-only confirmation container. The dashboard container receives only its database URL, dashboard storage token, and narrow dashboard→auth/confirmation caller capabilities; it receives no other pool or signing credential. Object access is provided by a Unix-socket-only broker. Storage and backup are bridge-networked, cannot reach IMDS, and consume separate short-lived bucket-role credentials. The only host-networked container is an input-free credential refresher; it assumes those roles from the instance profile and writes distinct read-only credential volumes.
